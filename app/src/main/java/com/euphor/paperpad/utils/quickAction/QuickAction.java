package com.euphor.paperpad.utils.quickAction;
...
public class QuickAction extends PopupWindows implements OnDismissListener {
	...
	public void addActionItem(ActionItem action) {
		...
		String duplicate = "";
		try {
			if (action.getCartItem().getDuplicate() > 1) {
				duplicate = " #" + action.getCartItem().getDuplicate();
			}
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		String title 	= action.getTitle()+duplicate;
		...
		if (action.isTitle()) {
			...
			text.setText(title);
			container.findViewById(R.id.TitleHolder).setBackgroundDrawable(colors.getForePD());//.setBackgroundColor(colors.getColor(colors.getForeground_color()));
			...
			text_price.setText(action.getCartItem().getPrice()+mContext.getString(R.string.euro));
