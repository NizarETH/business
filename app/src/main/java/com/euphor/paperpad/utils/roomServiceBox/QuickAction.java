package com.euphor.paperpad.utils.roomServiceBox;
...
public class QuickAction extends PopupWindows implements OnDismissListener {
    ...
    public QuickAction(Context context, int orientation, Colors colors) {
        ...
        if (mOrientation == HORIZONTAL) {
            ...
            setRootViewId(R.layout.popup_vertical_music);
        }
...
/*		if (action.isTitle()) {
			...
			if (mOrientation == HORIZONTAL) {
				container = mInflater.inflate(R.layout.action_item_room_horizontal, null);
			} else {
				container = mInflater.inflate(R.layout.action_item_room_vertical, null);
			}

//			ImageView img 	= (ImageView) container.findViewById(R.id.iv_icon);
			TextView text = (TextView) container.findViewById(R.id.tv_title);
//			ArrowImageView img_plus 	= (ArrowImageView) container.findViewById(R.id.iv_plus);
			TextView text_singer 	= (TextView) container.findViewById(R.id.tv_detail);
//			Paint paint = new Paint();
//			paint.setColor(colors.getColor(colors.getForeground_color(), "88"));
//			Drawable plusD = mContext.getResources().getDrawable(R.drawable.multi_select_d);
//			plusD.setColorFilter(colors.getColor(colors.getBody_color(), "88"), PorterDuff.Mode.MULTIPLY);
//			img_plus.setPaint(paint );
			NumberFormat nf = NumberFormat.getInstance(); // get instance
			nf.setMaximumFractionDigits(2); // set decimal places
			nf.setMinimumFractionDigits(2);
			
			String result = String.format(mContext.getResources().getString(R.string.detail_product_room_service), nf.format(action.getProduct().getUnit_price()), action.getProduct().getQuantity());
			text_singer.setText(result);
			text.setPadding(10, 10, 10, 10);
			text.setTextColor(Color.BLACK);
			text.setText(action.getProduct().getTitle());
			/*if (action.getPlaylist().getCover().isEmpty()) {
				img.setVisibility(View.INVISIBLE);
			}else {
				Glide.with(mContext).load(action.getPlaylist().getCover()).into(img);
			}
			if (icon != null) {
